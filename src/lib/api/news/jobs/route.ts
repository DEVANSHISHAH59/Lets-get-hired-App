...
// Fetches real live jobs from JSearch API (RapidAPI) which aggregates LinkedIn, Indeed, Glassdoor
// Falls back to curated static jobs if API is unavailable
  ...
  try {
    ...
    return allJobs.map((j: any) => ({
      ...
      salary: j.job_min_salary
        ? `EUR${Math.round(j.job_min_salary/1000)}k-${Math.round(j.job_max_salary/1000)}k`
        : 'Competitive',
      posted: j.job_posted_at_datetime_utc
        ? new Date(j.job_posted_at_datetime_utc).toLocaleDateString('en-IE')
        : 'Recent',
      url: j.job_apply_link || j.job_google_link,
      desc: j.job_description?.slice(0, 150) + '...' || '',
